    ...
    with open(data_path) as f:
        ...
        for line in f:
            ...
            if seen_entities and (not e1 in seen_entities or not e2 in seen_entities):
                num_skipped += 1
                if verbose:
                    print('Skip triple ({}) with unseen entity {} and entity {}'.format(num_skipped, e1, e2))
                continue
...
def load_triples(data_path, entity_index_path, relation_index_path, group_examples_by_query=False,
                 add_reverse_relations=False, seen_entities=None, verbose=False, few_shot=False, lf=None):
    """
            ...
            if group_examples_by_query:
                ...
                # few shot condition here:
                    ...
                    if r in lf.kg.few_shot_relation:
                        ...
                        if add_reverse_relations:
                            triple_inv = triple2ids(e1, e2, r+'_inv')
                            if triple_inv not in few_triples:
                    ...
                    # r -> normal_relation
                        ...
                        if add_reverse_relations:
                            triple_inv = triple2ids(e1, e2, r+'_inv')
                            if triple_inv not in normal_triples:
    ...
    if few_shot:
        print('{} normal_triples loaded from {}'.format(len(normal_triples), data_path))
        print('{} few_triples loaded from {}'.format(len(few_triples), data_path))
...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    ...
    for line in set(raw_kb_triples + keep_triples):
        ...
        if not triple_signature in removed_triples:
            ...
            # add entity2 to adj_list, (e1, r, e2) with their index (set by their frequency)
